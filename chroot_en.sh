...
echo -ne configuring pacman and the mirrorlist... && echo '[multilib]' >> /etc/pacman.conf && echo 'Include = /etc/pacman.d/mirrorlist' >> /etc/pacman.conf && reflector --country USA --latest 10 --protocol https --sort rate --save /etc/pacman.d/mirrorlist &> /dev/null && pacman -Syy &> /dev/null && echo done 
