echo -ne setting timezone... && ln -sf /usr/share/zoneinfo/Europe/Warsaw /etc/localtime && timedatectl --no-pager set-ntp true &> /dev/null && echo done

echo -ne setting hostname... && echo archbox > /etc/hostname && echo done

echo -ne generating locales... && echo "en_US.UTF-8 UTF-8" > /etc/locale.gen && echo "pl_PL.UTF-8 UTF-8" >> /etc/locale.gen && locale-gen &> /dev/null && echo 'LANG="pl_PL.UTF-8"' > /etc/locale.conf && echo done
